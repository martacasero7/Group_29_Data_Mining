...
\begin{table}[ht!]
    ...
    \includegraphics[width=0.4\linewidth]{cptree.png}
\end{figure}
\end{tabular}
    \caption{Decision tree and cp plot}
    \label{kaplan1}
\end{table}
...
\begin{table}[ht!]
    ...
    \begin{tabular}{c|c}
    \hline
    \includegraphics[width=0.4\linewidth]{Full tree.png} &
    \includegraphics[width=0.4\linewidth]{Full tree cp.png}
\end{figure}
\end{tabular}
    \caption{Pruned full tree and cp plot}
    \label{kaplan1}
\end{table}
...
kable(table( Actual=(actual<-train.data2$Crack),
  ...
  kable_styling(latex_options = "HOLD_position", full_width = F)
```
```{r lda_plot2, echo=FALSE, include=FALSE, eval = TRUE, warning=FALSE, fig.align = "center", fig.cap = "\\label{fig:corr} Linear discriminant analysis", fig.asp=0.7, fig.pos = "H"}
...
kable(table(actual<-train.data2$Crack, Predicted<-predict(qda.data2)$class)) %>%
  kable_styling(latex_options = "HOLD_position", full_width = F)
